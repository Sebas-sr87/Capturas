...
public class grafico extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            Boolean grafico_horas = request.getParameter("grafico_horas") == null ? Boolean.FALSE : Boolean.TRUE;
            JSONObject rj = new JSONObject();
            if (grafico_horas) {
                ;
                try {
                    List<Actividad> actividad = Query.Actividad();
                    if (actividad == null) {
                        throw new Exception();
                    } else {
                        try {
                            rj.put("actividad", actividad);
                        } catch (JSONException ex) {
                            out.println("traer datos de grafico actividades" + ex.toString());
                        }
